...
# PROJECT PATHS
# =========================================================
# ROOT represents the main project folder.
# DATA stores runtime files such as uploads, encrypted logs and timeline data.
...
# The main CSV log used by the dashboard is the timeline file from storage.py.
...
# CONFIGURATION
# =========================================================
# These values control the behavior of the system.
# They can also be updated from the web interface through /api/config.

CONFIG = {
    "mode": "dashboard",       # dashboard / camera / file
    "source_path": "",         # video path when file mode is selected
    "conf": 0.50,              # YOLO confidence threshold
    "resize_w": 1280,          # frame width used during analysis
    "tracker": "bytetrack.yaml",
    "use_pose": False,         # MediaPipe is optional and disabled by default

    # Rule-based behavior thresholds
    "speed_th": 90,            # minimum movement speed for RUN
    "dist_th": 85,             # distance threshold for close people
    "iou_th": 0.25,            # overlap threshold between person boxes
    "cooldown": 45,            # minimum frame gap before logging same person again

    # Multi-frame confirmation values
    # A behavior is not accepted from a single frame. It must continue
    # for a few frames to reduce false positives.
    "fight_hold": 4,
    "run_hold": 2,
    "fall_hold": 3,
}
...
# Only these behaviors are written to the timeline/log system.
# UNKNOWN, MOTIONLESS and LOITERING are not logged to keep the timeline clean.
...
def filter_duplicate_detections(detections):
    """
    Removes duplicate detections that represent the same person.
    This reduces the chance of drawing two boxes for one person.
    """
    kept = []

    for det in sorted(detections, key=lambda d: d.get("conf", 0), reverse=True):
        duplicate = False

        for old in kept:
            if bbox_iou(det["bbox"], old["bbox"]) > 0.65:
                duplicate = True
                break
...
def match_detection_to_person(bbox, used_ids):
    ...
    Purpose: reduce ID jumps and make RUN/FIGHT/FALL decisions more stable.
    """
    ...
    for pid, p in persons.items():
        ...
        max_dist = 130 if frames_missing <= 5 else 85

        if iou < 0.08 and d > max_dist:
            continue

        score = (iou * 3.0) + max(0.0, 1.0 - d / max_dist)
...
def classify_behavior(speed, overlaps, close_count, bbox, fight_hold, run_hold, fall_hold, pose_ok):
    """
    Rule-based behavior classifier.

    YOLO is responsible for detecting people.
    This function interprets movement patterns using simple measurable rules:
    - speed for running,
    - distance/overlap for fighting,
    - horizontal body box for falling.
    """
    x1, y1, x2, y2 = bbox
    w = max(1, x2 - x1)
    h = max(1, y2 - y1)
    ...
    # Abnormally large one-frame movement usually indicates ID instability.
    if speed > 240:
        return "UNKNOWN"

    # FALL: a person becomes horizontally positioned for several frames.
    if fall_hold >= CONFIG["fall_hold"] and speed <= 180:
        return "FALL"

    # Optional pose support can also confirm a possible fall.
    if pose_ok and aspect >= FALL_ASPECT and speed <= 180:
        return "FALL"

    # FIGHT: person is close/overlapping with someone else and moving.
    if (
        (overlaps >= 1 or close_count >= 1)
        and fight_hold >= CONFIG["fight_hold"]
        and 25 <= speed <= 180
    ):
        return "FIGHT"

    # RUN: person is moving fast without close interaction with another person.
    if (
        overlaps == 0
        and close_count == 0
        and run_hold >= CONFIG["run_hold"]
        and CONFIG["speed_th"] <= speed <= 220
    ):
        return "RUN"
